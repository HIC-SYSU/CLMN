#code from the Superpixel Segmentation with Fully Convolutional Networks
...
def init_spixel_grid(args,  b_train=True):
    ...
    all_XY_feat = (torch.from_numpy(np.tile(coord_tensor, (args.batch_size, 1, 1, 1)).astype(np.float32)).cuda())
